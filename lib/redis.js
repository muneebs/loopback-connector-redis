...
BridgeToRedis.prototype.all = function all(model, filter, callback) {
  ...
  if (filter.where) {
    ...
    var filterKey = Object.keys(filter.where);
    if (filter.where[filterKey]) {
      if (filter.where[filterKey].inq) {
        var sunion = [];
        filter.where[filterKey].inq.map(function (id) {
          sunion.push('i:' + model + ':' + filterKey[0] + ':' + id);
          // query.push(['SINTER', 'i:'+model+':'+filterKey[0]+':'+id]);
        });
        return client._client.send_command('sunion', sunion, function (err, replies) {
          if (err) return console.log('error', err);

          return handleKeys(null, replies.map(function (id) {
            return model + ':' + id;
          }));
        });
      }
